...
const AG_CATS = {
    azul:    { label: 'Saúde',    icon: 'stethoscope' },
    verde:   { label: 'Terapia',  icon: 'brain' },
    amarelo: { label: 'Educação', icon: 'graduation-cap' },
    rosa:    { label: 'Família',  icon: 'heart' },
    roxo:    { label: 'Remédio',  icon: 'pill' },
};
...
function _agRenderSemana(hoje) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        const dotCor = temEvento ? (eventos[0].cor || 'azul') : '';
        const cat = dotCor ? (AG_CATS[dotCor] || AG_CATS.azul) : null;
        const dot = temEvento && !eSel && !eHoje && cat
            ? `<div class="ag-week-day-icon ag-cat-${dotCor}"><i data-lucide="${cat.icon}"></i></div>`
...
function _agRenderTimeline() {
    ...
    tarefas.forEach((t, idx) => {
        ...
        porHora[h].push({ ...t, idx });
    });

    let html = '';
    for (let h = 0; h <= 23; h++) {
        const label = `${String(h).padStart(2,'0')}:00`;
        const eventos = porHora[h] || [];
        const blocos = eventos.map(t => {
            const range = t.fim ? `${t.hora} — ${t.fim}` : t.hora;
            const cat = AG_CATS[t.cor||'azul'] || AG_CATS.azul;
            return `<div class="ag-event-block ag-ev-${t.cor||'azul'}" onclick="event.stopPropagation()">
                ...
                <button class="ag-event-del" onclick="event.stopPropagation(); agDeletar('${chave}',${t.idx})"><i data-lucide="x"></i></button>
            </div>`;
        }).join('');
        html += `<div class="ag-time-row" onclick="agAbrirAdd('${label}')"><div class="ag-time-label">${label}</div><div class="ag-time-events">${blocos}</div></div>`;
    }
    el.innerHTML = html;
}
...
function agSalvarTarefa() {
    ...
    if (!hora || !texto) { alert('Preencha o horário e a atividade.'); return; }
    const la = JSON.parse(localStorage.getItem('la') || '{}');
