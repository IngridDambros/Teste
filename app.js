...
function ir(id) {
    ...
    if (telaAtiva && telaAtiva.id !== id) _telaAnterior = telaAtiva.id;
    ...
    if (id === 'tela-parceiros') { const tipos = ['educacao','inclusao','saude','terapia']; selecionarParcTab(Math.max(0, tipos.indexOf(_parcFiltroAtivo))); }
...
function _agRenderCompromissos() {
    ...
    /* ── Vista "Hoje" — timeline estilo app ── */
    if (ehHoje) {
        const azulCor = { amarelo:'#3D9EC9', verde:'#72C8EC', vermelho:'#2D8AB5', roxo:'#1F7BAA', azul:'#42A9D8' };
        const eventos = _AG_HOJE_MOCK; /* simulação — trocar por tarefas quando pronto */
        el.innerHTML = _agRenderHojeTimeline(eventos);
        lucide.createIcons();
        return;
    }

    const azulBanda = { amarelo:'#3D9EC9', verde:'#72C8EC', vermelho:'#2D8AB5', roxo:'#1F7BAA', azul:'#42A9D8' };
    const catLabel  = { amarelo:'Saúde', verde:'Educação', vermelho:'Família', roxo:'Terapia', azul:'Esporte' };

    if (tarefas.length === 0) {
        el.innerHTML = `<div class="ag-compromisso-vazio">
    ...
    el.innerHTML = tarefas.map((t, idx) => {
        const cat    = _agCat(t.cor || 'saude');
        const banda  = azulBanda[cat.cls] || '#3D9EC9';
        const label  = catLabel[cat.cls]  || cat.cls;
        const hora   = t.fim ? `${t.hora} — ${t.fim}` : t.hora;
        const nota   = t.detalhe ? ` · ${t.detalhe.slice(0, 28)}` : '';
        const [hh, mm] = (t.hora || '00:00').split(':');
        return `<div class="ag-banner-card" onclick="agAbrirDetalhe('${chave}',${idx})">
            <div class="ag-banner-left" style="background:${banda}">
                <span class="ag-banner-hora-h">${hh}</span>
            ...
            <div class="ag-banner-ico">
                <i data-lucide="${cat.icon}" style="width:22px;height:22px;stroke:${banda};stroke-width:1.8"></i>
            </div>
        </div>`;
    }).join('');
}
...
function fecharConfigFamilia() { irAnterior(); selecionarNivelTab(1); }
...
function fecharConfigSaude() { irAnterior(); selecionarNivelTab(2); }
...
function fecharConfigTerapia() { irAnterior(); selecionarNivelTab(3); }
...
function selecionarNivelTab(idx) {
    ...
    // Gráfico de evolução — só aparece na aba Educação
    const chartWrap = document.getElementById('chart-edu-wrap');
    if (chartWrap) {
        if (idx === 0) {
...
let _chartEduInstance = null;

function _renderChartEdu() {
    const canvas = document.getElementById('chart-edu');
    if (!canvas) return;
    ...
    const datasets = [
        {
            label: 'Conquistas e Marcas',
            data: [1, 2, 2, 4, 3, 5, 4, 6, 5, 7, 6, 8],
            cor: 'rgba(84,197,216,1)',
            corFill: 'rgba(84,197,216,0.5)',
        },
        {
            label: 'Cursos e Atividades',
            data: [0, 1, 2, 2, 3, 3, 5, 4, 6, 5, 7, 7],
            cor: 'rgba(82,183,136,1)',
            corFill: 'rgba(82,183,136,0.45)',
        },
        {
            label: 'Trajetória Escolar',
            data: [2, 2, 3, 3, 4, 4, 4, 5, 5, 6, 6, 7],
            cor: 'rgba(61,158,201,1)',
            corFill: 'rgba(61,158,201,0.4)',
        },
        {
            label: 'Vida Além da Sala',
            data: [0, 0, 1, 1, 2, 3, 3, 4, 4, 5, 5, 6],
            cor: 'rgba(92,107,192,1)',
            corFill: 'rgba(92,107,192,0.35)',
        },
    ];

    if (_chartEduInstance) {
        _chartEduInstance.destroy();
        _chartEduInstance = null;
    }

    _chartEduInstance = new Chart(canvas, {
        type: 'line',
        data: {
            labels: meses,
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    backgroundColor: 'rgba(26,58,92,.85)',
                    titleFont: { size: 11 },
                    bodyFont: { size: 11 },
                    padding: 8,
                    cornerRadius: 8,
            ...
            scales: {
                x: {
                    ...
                    ticks: { font: { size: 10 }, color: 'rgba(26,58,92,.4)' },
                },
                y: {
                    grid: { color: 'rgba(26,58,92,.06)', drawTicks: false },
                    border: { display: false, dash: [3, 3] },
                    ticks: { font: { size: 10 }, color: 'rgba(26,58,92,.4)', maxTicksLimit: 5 },
                    beginAtZero: true,
                },
            },
        },
    ...
    // Legenda manual
    const leg = document.getElementById('chart-edu-legenda');
    if (leg) {
        leg.innerHTML = datasets.map(d => `
            <div class="chart-leg-item">
                <div class="chart-leg-dot" style="background:${d.cor}"></div>
                <span>${d.label}</span>
            </div>`).join('');
    }
}
...
function buildNavHTML(activeDestino) {
    const items = [
        ...
        { id: 'tela-grafico',   icon: 'messages-square', label: 'Rede' },
        { id: 'tela-parceiros', icon: 'handshake',       label: 'Parceiros' },
    ];
    return items.map(it => {
...
function initTema() {
    const d = JSON.parse(localStorage.getItem('la') || '{}');
    // Migra tema escuro antigo para o padrão creme
    if (!d.tema || d.tema.cor === '#1a3a5c') {
        d.tema = { cor: '#F4EFE6', claro: true };
        localStorage.setItem('la', JSON.stringify(d));
    }
...
const REDE_CORES_TAB  = ['#3D9EC9','#1F7BAA','#2d83b0','#165F8C'];
const REDE_BG_CARDS   = ['#72C8EC','#5AB8E2','#3D9EC9','#2D8AB5','#1F7BAA','#165F8C','#1a3a5c'];
let _redeTabAtiva = 0;
...
const REDE_PILAR_CORES = ['#3D9EC9', '#1F7BAA', '#2d83b0', '#165F8C'];

function selecionarRedeTab(idx) {
    _redeTabAtiva = idx;
    const coresSlider = ['#72C8EC', '#3D9EC9', '#1F7BAA', '#165F8C'];
    const slider = document.getElementById('rede-nivel-slider');
    if (slider) {
        slider.style.transform = `translateX(${idx * 100}%)`;
        slider.style.background = coresSlider[idx];
    }
    document.querySelectorAll('#rede-nivel-tabs .pp-nivel-tab').forEach((t, i) =>
        t.classList.toggle('pp-nivel-tab-at', i === idx));

    const stack = document.getElementById('rede-content-cards');
    if (!stack) return;
    const temas = REDE_DATA[idx];
    const cor = REDE_PILAR_CORES[idx];
    const pilarNome = REDE_PILAR_NOMES[idx];

    const d = JSON.parse(localStorage.getItem('la') || '{}');

    stack.innerHTML = temas.map((t, i) => {
        const msgs = d[`rede_${t.chave}`] || [];
        const nMsgs = msgs.length;
        const lado = i % 2 === 0 ? 'left' : 'right';
        const bc = REDE_BG_CARDS[i % REDE_BG_CARDS.length];
        return `
        <div class="rl-bubble-wrap rl-bubble-${lado}">
            <div class="rl-bubble-av" style="background:${bc}">
...
function abrirSalaRede(chave, titulo) {
    ...
    const d = JSON.parse(localStorage.getItem('la') || '{}');
    const nomeUsuario = d.perfil?.['tea-nome'] ? 'Família de ' + d.perfil['tea-nome'] : 'Família anônima';
    const msgs = d[`rede_${chave}`] || [];

    const overlay = document.createElement('div');
    overlay.id = 'rl-sala-overlay';
    overlay.className = 'rl-sala';
    overlay.innerHTML = `
        <div class="rl-sala-wave-top" aria-hidden="true">
            <svg viewBox="0 0 390 110" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M0,0 L390,0 L390,76 C312,76 264,22 195,22 C126,22 78,76 0,76 Z" fill="#a8cce8" opacity=".5"/>
                <path d="M0,0 L390,0 L390,50 C312,50 268,12 195,12 C122,12 78,50 0,50 Z" fill="#5ca0c7" opacity=".3"/>
            </svg>
        </div>
        <div class="rl-sala-header">
            <button class="rl-sala-back" onclick="voltarRedeCards()">
                <i data-lucide="arrow-left" style="width:22px;height:22px;stroke:rgba(26,58,92,.65);stroke-width:1.8"></i>
            </button>
            <button class="pp-panel-plus" onclick="_headerPlus()">
                <i data-lucide="plus" style="width:20px;height:20px;stroke:rgba(26,58,92,.65);stroke-width:2.2"></i>
            </button>
        </div>
        <div class="rede-chat" id="rede-chat-feed">${_redeBubbleHtml(msgs, chave, nomeUsuario)}</div>
        <div id="rede-chat-input"></div>
        <div class="ag-onda-nav" aria-hidden="true">
            <svg viewBox="0 0 390 90" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M0,90 L390,90 L390,0 C312,0 260,30 195,30 C130,30 78,0 0,0 Z" fill="#a8cce8" opacity=".5"/>
                <path d="M0,90 L390,90 L390,16 C312,16 264,44 195,44 C126,44 78,16 0,16 Z" fill="#5ca0c7" opacity=".3"/>
            </svg>
        </div>
        <div class="ni pp-ni ag-nav-bottom">
            <div class="nit" onclick="voltarRedeCards();ir('tela-agenda')"><i data-lucide="calendar"></i></div>
            <div class="nit" onclick="voltarRedeCards();ir('tela-ajustes')"><i data-lucide="settings"></i></div>
            <div class="nit nit-logo" onclick="voltarRedeCards();ir('tela-painel')"><img src="LAazul.png" alt="Home" class="ni-la-logo"></div>
            <div class="nit at" onclick="voltarRedeCards()"><i data-lucide="messages-square"></i></div>
            <div class="nit" onclick="voltarRedeCards();ir('tela-parceiros')"><i data-lucide="handshake"></i></div>
        </div>`;
...
function abrirRedeLegado() { ir('tela-grafico'); }
...
function iniciarPerfil() {
    const d = JSON.parse(localStorage.getItem('la') || '{}');
    const p = d.perfil || {};
    const nomeEl  = document.getElementById('prf-hero-nome');
    const subEl   = document.getElementById('prf-hero-sub');
    const consEl  = document.getElementById('prf2-stat-consultas');
    const docsEl  = document.getElementById('prf2-stat-docs');
    if (nomeEl) nomeEl.textContent = p['tea-nome'] || p['resp1-nome'] || 'Usuário';
    if (subEl) {
        const vinculo = { mae:'Mãe', pai:'Pai', avo:'Avó/Avô', tutor:'Tutor(a)', outro:'Responsável' };
        const papel = vinculo[p['resp1-vinculo']] || 'Responsável';
        subEl.textContent = p['tea-nome'] ? `${papel} do(a) ${p['tea-nome']}` : 'Legado Azul';
    }
    if (consEl) {
        const agenda = d.agenda || {};
        const total = Object.values(agenda).reduce((s, arr) => s + (Array.isArray(arr) ? arr.length : 0), 0);
        consEl.textContent = total;
    }
    if (docsEl) docsEl.textContent = (d.documentos || []).length;
    lucide.createIcons();
}
...
function iniciarCfgTela() {
    ...
    try {
        const d = JSON.parse(localStorage.getItem('la') || '{}');
        const nome = d.respNome || d.teaNome || 'Usuário';
        const partes = nome.trim().split(/\s+/);
        const iniciais = partes.length >= 2
            ? (partes[0][0] + partes[partes.length - 1][0]).toUpperCase()
            : nome.slice(0, 2).toUpperCase();
        const avatarEl = document.getElementById('cfg-avatar-initials');
        const nomeEl   = document.getElementById('cfg-user-nome');
        if (avatarEl) avatarEl.textContent = iniciais;
        if (nomeEl)   nomeEl.textContent   = nome;
        const tema = d.tema;
        const modoEscuroEl = document.getElementById('cfg-modo-escuro');
...
function toggleModoEscuro(escuro) {
    ...
    if (escuro) {
        ...
        aplicarTema(tema.corClara || '#F4EFE6', true);
    }
}
...
const PARCEIROS = {
    ...
    inclusao: [
        { id:'inc-apae',  nome:'APAE / Associação', icone:'users',    cor:_PARC_COR, rgba:_PARC_RGBA },
        { id:'inc-cras',  nome:'CRAS / CREAS',      icone:'landmark', cor:_PARC_COR, rgba:_PARC_RGBA },
        { id:'inc-ong',   nome:'ONG / Instituto',   icone:'globe',    cor:_PARC_COR, rgba:_PARC_RGBA },
        { id:'inc-trans', nome:'Transporte',         icone:'bus',      cor:_PARC_COR, rgba:_PARC_RGBA },
    ],
    trabalho: [
...
const _PARC_CAT_LABEL = {
    educacao:'Educação', saude:'Saúde', terapia:'Terapia', inclusao:'Inclusão', trabalho:'Trabalho'
};
...
function selecionarParcTab(idx) {
    const cores = ['#72C8EC', '#3D9EC9', '#1F7BAA', '#1a3a5c'];
    const tipos = ['educacao', 'inclusao', 'saude', 'terapia'];
    const slider = document.getElementById('parc-nivel-slider');
    if (slider) {
        slider.style.transform = `translateX(${idx * 100}%)`;
        slider.style.background = cores[idx];
    }
    document.querySelectorAll('#parc-filtro-tabs .pp-nivel-tab').forEach((t, i) =>
        t.classList.toggle('pp-nivel-tab-at', i === idx)
    );
...
function _parcRenderizar(filtro) {
    ...
    if (!lista) return;
    const grupos = _PARC_GRUPOS[filtro] || _PARC_GRUPOS.todos;
    let todos = [];
    grupos.forEach(g => { (PARCEIROS[g] || []).forEach(p => todos.push({...p, cat: g})); });

    const coresGrad = ['#72C8EC','#5AB8E2','#42A9D8','#3D9EC9','#2D8AB5','#1F7BAA','#165F8C'];

    lista.innerHTML = `<div class="parc-grid">${todos.map((p, i) => {
        const bg = coresGrad[i % coresGrad.length];
        const desc = _PARC_DESC[p.id] || 'Parceiros especializados para apoiar sua família.';
        return `<div class="parc-grid-card" style="background:${bg}" onclick="abrirFicha('${p.id}')">
            <div class="parc-grid-ico" style="background:rgba(255,255,255,.22)">
                <i data-lucide="${p.icone}" style="width:34px;height:34px;stroke:white;stroke-width:1.5"></i>
            </div>
            <div class="parc-grid-nome" style="color:white">${p.nome}</div>
            <div class="parc-grid-desc" style="color:rgba(255,255,255,.80)">${desc}</div>
            <button class="parc-grid-btn" style="background:rgba(255,255,255,.22)" onclick="event.stopPropagation();abrirFicha('${p.id}')">
                <i data-lucide="chevron-right" style="width:15px;height:15px;stroke:white;stroke-width:2.5"></i>
            </button>
        </div>`;
    }).join('')}</div>`;
    lucide.createIcons();
...
function abrirFicha(id, nivelOrigem) {
    ...
    const allList = [...PARCEIROS.educacao, ...PARCEIROS.inclusao, ...PARCEIROS.saude, ...PARCEIROS.terapia];
    const p = allList.find(x => x.id === id);
    if (!p) return;
    const coresGrad = ['#72C8EC','#5AB8E2','#42A9D8','#3D9EC9','#2D8AB5','#1F7BAA','#165F8C'];
    const idx = allList.findIndex(x => x.id === id);
    const bg = coresGrad[idx % coresGrad.length];
    const desc = _PARC_DESC[id] || 'Parceiros especializados para apoiar sua família.';
    document.getElementById('parc-ficha-body').innerHTML = `
        <div class="pf-hero">
            <div class="pf-hero-ico" style="background:${bg}">
                <i data-lucide="${p.icone}" style="width:48px;height:48px;stroke:white;stroke-width:1.4"></i>
            </div>
            <div class="pf-hero-nome">${p.nome}</div>
            <div class="pf-hero-desc">${desc}</div>
        </div>
        <div class="pf-em-breve">
            <div class="pf-eb-ico">
                <i data-lucide="clock" style="width:28px;height:28px;stroke:#3D9EC9;stroke-width:1.5"></i>
            </div>
            <p class="pf-eb-txt">Em breve, profissionais e instituições cadastradas nessa especialidade aparecerão aqui.</p>
        </div>
        <button class="pf-cta-btn" style="background:${bg}" onclick="mostrarToast('Cadastro de parceiros em breve!')">
            <i data-lucide="handshake" style="width:18px;height:18px;stroke:white;stroke-width:1.8"></i>
            Quero ser parceiro
        </button>
    `;
    document.querySelectorAll('.tela, .subtela').forEach(t => t.classList.remove('ativa'));
...
function selecionarAjustesTab(idx) {
    const cores = ['#72C8EC', '#3D9EC9', '#1F7BAA', '#1a3a5c'];
    const slider = document.getElementById('cfg-nivel-slider');
    if (slider) {
        slider.style.transform = `translateX(${idx * 100}%)`;
        slider.style.background = cores[idx];
    }
    document.querySelectorAll('#cfg-filtro-tabs .pp-nivel-tab').forEach((t, i) =>
        t.classList.toggle('pp-nivel-tab-at', i === idx)
    );
    const grupos = [
        ['cfg-sec-perfil', 'cfg-card-perfil'],
        ['cfg-sec-docs',   'cfg-card-docs'],
        ['cfg-sec-graf',   'cfg-card-graf'],
        ['cfg-sec-seg',    'cfg-card-seg'],
    ];
