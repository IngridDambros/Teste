...
function ir(id) {
    ...
    lucide.createIcons();
    if (id === 'tela-painel') { atuConts(); _atuSaudacao(); }
...
function _agRenderSemana() {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        const cor     = corDia[diaSem];

        let evNome = 'Livre';
        let evSub  = '';
        let evIcon = 'sun';
        let evCor  = 'rgba(26,58,92,.25)';

        if (tarefas.length > 0) {
            const cat = _agCat(tarefas[0].cor || 'saude');
            evNome = tarefas.length === 1 ? tarefas[0].texto : `${tarefas[0].texto} +${tarefas.length - 1}`;
            evSub  = tarefas[0].hora + (tarefas[0].fim ? ` — ${tarefas[0].fim}` : '');
            evIcon = cat.icon;
            evCor  = corBanda[cat.cls] || cor;
        }

        const ano = data.getFullYear(), mes = data.getMonth(), dia = data.getDate();
        html += `<div class="ag-week-card-wrap${ehHoje ? ' ag-week-hoje' : ''}" onclick="_agVerDia(new Date(${ano},${mes},${dia}))">
            <span class="ag-week-num">${dia}</span>
            <div class="ag-week-card">
                <div class="ag-week-left" style="background:${cor}${ehHoje ? ';filter:brightness(.85)' : ''}">
                    <span class="ag-week-dia-nome">${diasNome[diaSem]}</span>
                </div>
                <div class="ag-week-body">
                    <div class="ag-week-ev-nome${tarefas.length === 0 ? ' ag-week-livre' : ''}">${evNome}</div>
                    ${evSub ? `<div class="ag-week-ev-sub">${evSub}</div>` : ''}
                </div>
                <div class="ag-week-ico">
                    <i data-lucide="${evIcon}" style="width:22px;height:22px;stroke:${evCor};stroke-width:1.8"></i>
                </div>
            </div>
        </div>`;
    }
...
══════════════════════════════════════════ */
const _EDU_CAMPOS = [
    // Educação formal — ordem alfabética
    { id:'conquistas',        nome:'Conquistas & Prêmios',               grupo:'formal', icon:'trophy' },
    { id:'curso-tecnico',     nome:'Curso técnico',                      grupo:'formal', icon:'wrench' },
    { id:'cursos-livres',     nome:'Cursos livres',                      grupo:'formal', icon:'bookmark' },
    { id:'eja',               nome:'EJA — Educação de Jovens e Adultos', grupo:'formal', icon:'users' },
    { id:'escola-primaria',   nome:'Escola primária',                    grupo:'formal', icon:'school' },
    { id:'escola-secundaria', nome:'Escola secundária / Ens. médio',     grupo:'formal', icon:'book' },
    { id:'faculdade',         nome:'Faculdade / Universidade',           grupo:'formal', icon:'graduation-cap' },
    { id:'formatura',         nome:'Formatura',                          grupo:'formal', icon:'star' },
    { id:'intercambio',       nome:'Intercâmbio',                        grupo:'formal', icon:'plane' },
    { id:'pos-graduacao',     nome:'Pós-graduação / MBA',                grupo:'formal', icon:'award' },
    { id:'professor',         nome:'Professor marcante',                  grupo:'formal', icon:'heart' },
...
const _FAM_CAMPOS = [
    { id:'album',        nome:'Álbum de fotos',                  icon:'image' },
    { id:'aniversarios', nome:'Aniversários e datas especiais',  icon:'cake' },
    { id:'arvore',       nome:'Árvore genealógica',              icon:'git-branch' },
    { id:'carta',        nome:'Carta para o futuro',             icon:'mail' },
    { id:'conquistas',   nome:'Conquistas em família',           icon:'trophy' },
    { id:'dinamica',     nome:'Dinâmica familiar',               icon:'users' },
    { id:'infancia',     nome:'Histórias de infância',           icon:'book-open' },
    { id:'memorias',     nome:'Memórias afetivas',               icon:'heart' },
    { id:'receitas',     nome:'Receitas da família',             icon:'utensils' },
    { id:'tradicoes',    nome:'Tradições e costumes',            icon:'star' },
    { id:'valores',      nome:'Valores familiares',              icon:'shield' },
    { id:'viagens',      nome:'Viagens em família',              icon:'map-pin' },
];

const _SAU_CAMPOS = [
    { id:'alergias',     nome:'Alergias e restrições',           icon:'alert-triangle' },
    { id:'consultas',    nome:'Consultas médicas',               icon:'stethoscope' },
    { id:'motor',        nome:'Desenvolvimento motor',           icon:'activity' },
    { id:'alimentacao',  nome:'Dieta e alimentação',             icon:'apple' },
    { id:'crises',       nome:'Episódios de crise',              icon:'zap' },
    { id:'exames',       nome:'Exames e laudos',                 icon:'file-text' },
    { id:'internacoes',  nome:'Histórico de internações',        icon:'building-2' },
    { id:'medicamentos', nome:'Medicamentos em uso',             icon:'pill' },
    { id:'comportamento',nome:'Progresso comportamental',        icon:'trending-up' },
    { id:'sono',         nome:'Rotina de sono',                  icon:'moon' },
    { id:'bucal',        nome:'Saúde bucal',                     icon:'smile' },
    { id:'vacinas',      nome:'Vacinas',                         icon:'syringe' },
];

const _TER_CAMPOS = [
    { id:'aba',          nome:'ABA — Análise do Comportamento',  icon:'bar-chart-2' },
    { id:'caa',          nome:'Comunicação alternativa',         icon:'message-circle' },
    { id:'fisio',        nome:'Fisioterapia',                    icon:'person-standing' },
    { id:'fono',         nome:'Fonoaudiologia',                  icon:'mic' },
    { id:'social',       nome:'Habilidades sociais',             icon:'handshake' },
    { id:'sensorial',    nome:'Integração sensorial',            icon:'layers' },
    { id:'metas',        nome:'Metas terapêuticas',              icon:'target' },
    { id:'musico',       nome:'Musicoterapia',                   icon:'music' },
    { id:'objetivos',    nome:'Objetivos de curto prazo',        icon:'check-circle' },
    { id:'progresso',    nome:'Progresso mensal',                icon:'calendar' },
    { id:'psico',        nome:'Psicologia',                      icon:'brain' },
    { id:'to',           nome:'Terapia Ocupacional',             icon:'wrench' },
];
...
function togglePilarCampo(pilar, id, chk) {
    const d = JSON.parse(localStorage.getItem('la') || '{}');
    const key = pilar + '_campos';
    const sel = d[key] || [];
    const idx = sel.indexOf(id);
    const on = chk ? chk.checked : idx === -1;
...
function _renderPilarToggleHtml(pilar, camposArr, cor) {
    ...
    return camposArr.map(c => {
        ...
        return `<div class="edu-toggle-item">
            <div class="edu-toggle-ico" style="background:${_hexToRgba(cor,.12)}">
                <i data-lucide="${c.icon}" style="width:16px;height:16px;stroke:${cor};stroke-width:1.7"></i>
            </div>
...
function _toggleItemHtml(pilar, c, cor) {
    ...
    return `<div class="edu-toggle-item">
        ...
        <span class="edu-toggle-nome">${c.nome}</span>
        <label class="fe-toggle-sw" style="cursor:pointer">
            <input type="checkbox" ${on ? 'checked' : ''} onchange="togglePilarCampo('${pilar}','${c.id}',this)">
            <span class="fe-toggle-track"></span>
        </label>
    </div>`;
...
function _abrirSeletorPilar(pilar) {
    ...
    let conteudo;
    if (pilar === 'educacao') {
        const formalHtml = _EDU_CAMPOS.filter(c => c.grupo === 'formal').map(c => _toggleItemHtml('educacao', c, '#3A9A6A')).join('');
        const vidaHtml   = _EDU_CAMPOS.filter(c => c.grupo === 'vida').map(c => _toggleItemHtml('educacao', c, '#3A9A6A')).join('');
        conteudo = `<div class="pil-sel-group-title">Educação Formal</div>${formalHtml}<div class="pil-sel-group-title">Aprendizado de Vida</div>${vidaHtml}`;
    } else {
        const camposArr = _PILAR_CAMPOS_MAP[pilar] || [];
    ...
    overlay.innerHTML = `<div class="pil-seletor">
        <div class="pp2-header" style="flex-shrink:0">
            <span class="pil-sel-titulo">${pilarInfo?.nome || 'Campos'}</span>
    ...
    overlay.innerHTML = `<div class="pil-form-screen">
        <div class="pp2-header" style="flex-shrink:0">
            ...
            <div style="flex:1;text-align:center;padding:0 8px;min-width:0">
                ...
                <div style="font-family:var(--ft);font-size:.65rem;color:${cor};letter-spacing:.03em">${pilarInfo?.nome || ''}</div>
            </div>
            <img src="LAbranco.png" alt="Legado Azul" class="pp2-logo">
        </div>
...
function renderFamilia() {
    ...
    if (!sec) return;
    sec.innerHTML = _renderPilarCardsHtml('familia', '#C84040');
    lucide.createIcons();
}
function renderSaude() {
    const sec = document.getElementById('sau-cards');
    if (!sec) return;
    sec.innerHTML = _renderPilarCardsHtml('saude', '#C8A020');
    lucide.createIcons();
}
function renderTerapia() {
    const sec = document.getElementById('ter-cards');
    if (!sec) return;
    sec.innerHTML = _renderPilarCardsHtml('terapia', '#7855C8');
    lucide.createIcons();
}
...
function toggleEduCampo(id, chk) {
    const d = JSON.parse(localStorage.getItem('la') || '{}');
...
function _eduPillsHtml(grupo) {
    ...
    return _EDU_CAMPOS.filter(c => c.grupo === grupo).map(c => {
        const on = sel.includes(c.id);
        return `<div class="edu-toggle-item" id="edu-item-${c.id}">
            <div class="edu-toggle-ico">
                <i data-lucide="${c.icon}" style="width:16px;height:16px;stroke:#3A9A6A;stroke-width:1.7"></i>
            </div>
            <span class="edu-toggle-nome">${c.nome}</span>
            <label class="fe-toggle-sw" style="cursor:pointer">
                <input type="checkbox" ${on ? 'checked' : ''} onchange="toggleEduCampo('${c.id}',this)">
                <span class="fe-toggle-track"></span>
            </label>
        </div>`;
    }).join('');
...
function renderEducacao() {
    ...
    if (!sec) return;
    sec.innerHTML = _renderPilarCardsHtml('educacao', '#3A9A6A');
    lucide.createIcons();
}
...
const MSG_RESPONSAVEIS = {
    ...
    familia: [
        { chave:'psico_fam',  titulo:'Psicólogo(a)',      desc:'Apoio emocional e familiar',      icon:'brain' },
        { chave:'assistente', titulo:'Assistente Social', desc:'Orientação e recursos sociais',   icon:'heart-handshake' },
    ],
    saude: [
...
function _renderMensagensTab(idx) {
    ...
    if (!stack) return;
    const responsaveis = MSG_RESPONSAVEIS[pilarKey] || [];
    const shades = REDE_PILAR_SHADES[idx];
    const d = JSON.parse(localStorage.getItem('la') || '{}');

    stack.innerHTML = responsaveis.map((r, i) => {
        ...
        return `
        <div class="rl-bubble-wrap rl-bubble-${lado}">
            <div class="rl-bubble-av">
                <i data-lucide="${r.icon}" style="width:24px;height:24px;stroke:${bc};stroke-width:1.7;fill:none"></i>
            </div>
            <div class="rl-bubble" style="--bc:${bc}" onclick="_abrirChatResponsavel('${r.chave}','${r.titulo}','${pilarInfo?.nome}',${idx})">
                <div class="rl-bubble-nome">${r.titulo}</div>
                <div class="rl-bubble-desc">${r.desc}</div>
                <div class="rl-bubble-foot">
...
function _abrirChatResponsavel(chave, titulo, subTitulo, pilarIdx) {
    ...
    overlay.innerHTML = `
        <div class="pp2-header com-header">
            <button class="pp2-nav-btn" onclick="voltarMsgChat()" style="margin-left:-8px">
                <i data-lucide="arrow-left" style="width:24px;height:24px;stroke:rgba(26,58,92,.55);stroke-width:2;fill:none"></i>
            </button>
            <div class="com-header-title">
                <div class="com-canal-nome-txt">${titulo}</div>
                <div class="com-canal-sub-txt">${subTitulo}</div>
            </div>
            <img src="LAbranco.png" alt="Legado Azul" class="pp2-logo">
        </div>
...
function selecionarRedeTab(idx) {
    ...
    stack.innerHTML = temas.map((t, i) => {
        ...
        <div class="rl-bubble-wrap rl-bubble-${lado}">
            <div class="rl-bubble-av">
                <i data-lucide="${t.icon}" style="width:24px;height:24px;stroke:${bc};stroke-width:1.7;fill:none"></i>
            </div>
            <div class="rl-bubble" style="--bc:${bc}" onclick="abrirSalaRede('${t.chave}','${t.titulo}')">
                <div class="rl-bubble-nome">${t.titulo}</div>
                <div class="rl-bubble-desc">${t.desc}</div>
                <div class="rl-bubble-foot">
                    <i data-lucide="message-circle"></i>
                    <span>${nMsgs} ${nMsgs === 1 ? 'mensagem' : 'mensagens'}</span>
                </div>
            </div>
        </div>`;
    }).join('');
...
function abrirSalaRede(chave, titulo) {
    ...
    overlay.innerHTML = `
        <div class="pp2-header com-header">
            <button class="pp2-nav-btn" onclick="voltarRedeCards()" style="margin-left:-8px">
                <i data-lucide="arrow-left" style="width:24px;height:24px;stroke:rgba(26,58,92,.55);stroke-width:2"></i>
            </button>
            <div class="com-header-title">
                <div class="com-canal-nome-txt">${titulo}</div>
                <div class="com-canal-sub-txt">Rede Legado</div>
            </div>
            <img src="LAbranco.png" alt="Legado Azul" class="pp2-logo">
        </div>
...
function criarTelaRedeLegado() { ir('tela-grafico'); }

function fecharRedeLegado() { ir('tela-painel'); }
...
function selecionarAjustesTab(idx) {
    _ajTabAtiva = idx;
    const slider = document.getElementById('aj-nivel-slider');
    if (slider) { slider.style.transform = `translateX(${idx * 100}%)`; slider.style.background = AJ_CORES_TAB[idx]; }
    document.querySelectorAll('#aj-nivel-tabs .pp-nivel-tab').forEach((t, i) =>
        t.classList.toggle('pp-nivel-tab-at', i === idx));
    const stack = document.getElementById('ajustes-content-cards');
    if (!stack) return;
    stack.innerHTML = AJUSTES_DATA[idx].map((t, i) => `
        <div class="pp-panel-card ${AJ_CARD_CLS[i]}" onclick="abrirCardAjuste(this,'${t.tipo}')">
            <div class="pp-pc-topo">
                <h2 class="pp-pc-titulo">${t.titulo}</h2>
                <i data-lucide="${t.icon}" style="width:20px;height:20px;stroke:rgba(255,255,255,0.65);stroke-width:2;flex-shrink:0"></i>
            </div>
            <p class="pp-pc-desc">${t.desc}</p>
        </div>`).join('');
    lucide.createIcons();
}
...
function criarTelaRedeLegado() {
    ...
    tela.innerHTML = `
        <div class="rede-header" style="flex-shrink:0">
            <button class="bv bv-white" onclick="fecharRedeLegado()" style="flex-shrink:0">
...
const PARCEIROS = {
    educacao: [
        { id:'p-edu-01', nome:'Centro Educacional Inclusivo', subTitulo:'Educação Especial',   rating:4.8, icone:'graduation-cap', cor:'#3A9A6A', thumbBg:'rgba(58,154,106,.12)',  desc:'Atendimento educacional especializado para crianças neurodivergentes.' },
        { id:'p-edu-02', nome:'Instituto de Mediação Escolar', subTitulo:'Mediação & Apoio',   rating:4.7, icone:'users',          cor:'#3A9A6A', thumbBg:'rgba(58,154,106,.12)',  desc:'Mediadores capacitados para apoio em sala de aula regular.' },
        { id:'p-edu-03', nome:'Escola de Aprendizagem Viva',  subTitulo:'Educação',            rating:4.9, icone:'book-open',      cor:'#3A9A6A', thumbBg:'rgba(58,154,106,.12)',  desc:'Metodologia ativa e inclusiva para o desenvolvimento infantil.' },
        { id:'p-edu-04', nome:'Cursos de Desenvolvimento',    subTitulo:'Capacitação',         rating:4.6, icone:'laptop',         cor:'#3A9A6A', thumbBg:'rgba(58,154,106,.12)',  desc:'Cursos livres e profissionalizantes para pais e cuidadores.' },
    ],
    familia: [
        { id:'p-fam-01', nome:'APAE Regional',         subTitulo:'Família & Inclusão',    rating:4.9, icone:'heart',    cor:'#C84040', thumbBg:'rgba(200,64,64,.12)',   desc:'Associação de apoio à pessoa com deficiência e suas famílias.' },
        { id:'p-fam-02', nome:'CRAS Família Presente', subTitulo:'Apoio Social',          rating:4.7, icone:'home',     cor:'#C84040', thumbBg:'rgba(200,64,64,.12)',   desc:'Serviços socioassistenciais e proteção à família.' },
        { id:'p-fam-03', nome:'Instituto Família Forte',subTitulo:'Família',              rating:4.8, icone:'users',    cor:'#C84040', thumbBg:'rgba(200,64,64,.12)',   desc:'Suporte emocional e orientação para famílias de neurodivergentes.' },
        { id:'p-fam-04', nome:'ONG Rede Solidária',    subTitulo:'Família & Comunidade', rating:4.6, icone:'globe',    cor:'#C84040', thumbBg:'rgba(200,64,64,.12)',   desc:'Rede de apoio comunitário para famílias em situação de vulnerabilidade.' },
    ],
    saude: [
        { id:'p-sau-01', nome:'Centro Neuroclínico',    subTitulo:'Saúde & Terapia',    rating:4.8, icone:'brain',      cor:'#C8A020', thumbBg:'rgba(200,160,32,.12)', desc:'Diagnóstico e acompanhamento neurológico para crianças e adultos.' },
        { id:'p-sau-02', nome:'Clínica Pediátrica Viver',subTitulo:'Pediatria',         rating:4.9, icone:'baby',       cor:'#C8A020', thumbBg:'rgba(200,160,32,.12)', desc:'Acompanhamento pediátrico especializado em desenvolvimento infantil.' },
        { id:'p-sau-03', nome:'Neurologia Infantil',   subTitulo:'Saúde Neurológica',  rating:4.7, icone:'activity',    cor:'#C8A020', thumbBg:'rgba(200,160,32,.12)', desc:'Consultas e exames neurológicos para crianças e adolescentes.' },
        { id:'p-sau-04', nome:'Odontologia Especial',  subTitulo:'Saúde Bucal',        rating:4.6, icone:'smile',       cor:'#C8A020', thumbBg:'rgba(200,160,32,.12)', desc:'Atendimento odontológico adaptado para pacientes especiais.' },
    ],
    terapia: [
        { id:'p-ter-01', nome:'Clínica ABA Avançada',    subTitulo:'Terapia Comportamental', rating:4.9, icone:'target',  cor:'#7855C8', thumbBg:'rgba(120,85,200,.12)', desc:'Análise do comportamento aplicada com profissionais certificados.' },
        { id:'p-ter-02', nome:'Studio de Fonoaudiologia', subTitulo:'Terapia',               rating:4.8, icone:'mic',     cor:'#7855C8', thumbBg:'rgba(120,85,200,.12)', desc:'Avaliação e tratamento da comunicação, linguagem e deglutição.' },
        { id:'p-ter-03', nome:'Espaço TO & Fisioterapia', subTitulo:'Terapia',               rating:4.7, icone:'dumbbell',cor:'#7855C8', thumbBg:'rgba(120,85,200,.12)', desc:'Terapia ocupacional e fisioterapia com foco no desenvolvimento motor.' },
        { id:'p-ter-04', nome:'Centro de Psicologia',     subTitulo:'Psicologia',            rating:4.8, icone:'brain',   cor:'#7855C8', thumbBg:'rgba(120,85,200,.12)', desc:'Suporte psicológico especializado para toda a família.' },
    ],
};
...
function _parcRenderizar(filtro) {
    ...
    lista.innerHTML = parceiros.map(p => `
        <div class="parc-list-card" onclick="abrirFicha('${p.id}')">
            <div class="parc-list-info">
                <div class="parc-list-nome">${p.nome}</div>
                <div class="parc-list-cat">${p.subTitulo}</div>
                <div class="parc-list-rating">
                    <i data-lucide="star" style="width:11px;height:11px;stroke:#C8A020;fill:#C8A020;stroke-width:0"></i>
                    <span>${p.rating.toFixed(1)}</span>
                </div>
            </div>
            <div class="parc-list-arrow">
                <i data-lucide="chevron-right" style="width:18px;height:18px;stroke:rgba(30,58,71,.3);stroke-width:2"></i>
            </div>
        </div>
    `).join('');
    lucide.createIcons();
...
document.addEventListener('click', function(e) {
    if (e.target.classList.contains('pp2-logo')) ir('tela-painel');
});
