...
function voltarDePlanos() { ir('tela-login'); if (_perfilSelecionado) abrirMo(); }
...
function atuConts() {
    ...
    Object.entries(map).forEach(([p,id])=>{let t=0;Object.values(d[p]||{}).forEach(a=>{t+=a.length;});const el=document.getElementById(id);if(el) el.textContent=t+(t===1?' registro':' registros');});
...
window.onload = () => {
    ...
    document.querySelectorAll('.nit').forEach(item=>{
        item.addEventListener('click',()=>{
            ...
            if(destino){document.querySelectorAll('.nit').forEach(n=>n.classList.remove('at'));item.classList.add('at');ir(destino);}
        });
    });
...
══════════════════════════════════════════ */
function abrirFormSaude(id){_fecharRede();document.getElementById('subtela-saude').classList.remove('ativa');document.getElementById(id).classList.add('ativa');lucide.createIcons();}
function fecharFormSaude(id){document.getElementById(id).classList.remove('ativa');document.getElementById('subtela-saude').classList.add('ativa');lucide.createIcons();}
function salvarFormSaude(id){const form=document.getElementById(id);const titulo=form.querySelector('.sti')?form.querySelector('.sti').textContent:id;const campos={};form.querySelectorAll('.fin,.fsel').forEach(el=>{const label=el.closest('.frow')?.querySelector('.fla')?.textContent||'';if(el.value&&label) campos[label]=el.value;});const d=JSON.parse(localStorage.getItem('la')||'{}');if(!d.saude) d.saude={};if(!d.saude[id]) d.saude[id]=[];d.saude[id].push({titulo,campos,data:new Date().toISOString(),pilar:'saude'});localStorage.setItem('la',JSON.stringify(d));fecharFormSaude(id);atuConts();enviarNotificacaoLocal('✅ Registro salvo',titulo);}

function abrirFormTerapia(id){_fecharRede();document.getElementById('subtela-terapia').classList.remove('ativa');document.getElementById(id).classList.add('ativa');lucide.createIcons();}
function fecharFormTerapia(id){document.getElementById(id).classList.remove('ativa');document.getElementById('subtela-terapia').classList.add('ativa');lucide.createIcons();}
function salvarFormTerapia(id){const form=document.getElementById(id);const titulo=form.querySelector('.sti')?form.querySelector('.sti').textContent:id;const campos={};form.querySelectorAll('.fin,.fsel').forEach(el=>{const label=el.closest('.frow')?.querySelector('.fla')?.textContent||'';if(el.value&&label) campos[label]=el.value;});const d=JSON.parse(localStorage.getItem('la')||'{}');if(!d.terapia) d.terapia={};if(!d.terapia[id]) d.terapia[id]=[];d.terapia[id].push({titulo,campos,data:new Date().toISOString(),pilar:'terapia'});localStorage.setItem('la',JSON.stringify(d));fecharFormTerapia(id);atuConts();enviarNotificacaoLocal('✅ Registro salvo',titulo);}

function abrirFormFamilia(id){_fecharRede();document.getElementById('subtela-familia').classList.remove('ativa');document.getElementById(id).classList.add('ativa');lucide.createIcons();}
function fecharFormFamilia(id){document.getElementById(id).classList.remove('ativa');document.getElementById('subtela-familia').classList.add('ativa');lucide.createIcons();}
function salvarFormFamilia(id){const form=document.getElementById(id);const titulo=form.querySelector('.sti')?form.querySelector('.sti').textContent:id;const campos={};form.querySelectorAll('.fin,.fsel').forEach(el=>{const label=el.closest('.frow')?.querySelector('.fla')?.textContent||'';if(el.value&&label) campos[label]=el.value;});const d=JSON.parse(localStorage.getItem('la')||'{}');if(!d.familia) d.familia={};if(!d.familia[id]) d.familia[id]=[];d.familia[id].push({titulo,campos,data:new Date().toISOString(),pilar:'familia'});localStorage.setItem('la',JSON.stringify(d));fecharFormFamilia(id);atuConts();enviarNotificacaoLocal('✅ Registro salvo',titulo);}
...
function confirmarSair(){if(confirm('Deseja sair da sua conta?')) ir('tela-login');}

/* ── Educação status ── */
function setEduStatus(status){document.querySelectorAll('.edu-btn').forEach(b=>b.classList.remove('at'));document.getElementById('ebtn-'+status).classList.add('at');['matriculado','nao-matriculado','adaptado'].forEach(s=>{const el=document.getElementById('edu-'+s);if(el) el.classList.toggle('oculto',s!==status);});try{const d=JSON.parse(localStorage.getItem('la')||'{}');d.eduStatus=status;localStorage.setItem('la',JSON.stringify(d));}catch(e){}}
...
const _credenciais={medico:{titulo:'Suas credenciais — Saúde',sub:'Informe seu registro para verificação',campos:[{label:'CRM',placeholder:'CRM 000000 / UF',tipo:'text'},{label:'Especialidade',placeholder:'Ex: Neuropediatria, Psiquiatria...',tipo:'text'},{label:'Instituição / Clínica',placeholder:'Nome da instituição (opcional)',tipo:'text'}]},terapeuta:{titulo:'Suas credenciais — Terapia',sub:'Informe seu registro para verificação',campos:[{label:'Conselho e registro',placeholder:'Ex: CRP 00000, CREFITO 00000...',tipo:'text'},{label:'Especialidade / Abordagem',placeholder:'Ex: TO, Fonoaudiologia, Psicologia...',tipo:'text'},{label:'Instituição / Clínica',placeholder:'Nome da instituição (opcional)',tipo:'text'}]},escola:{titulo:'Suas credenciais — Escola',sub:'Informe os dados da instituição',campos:[{label:'Nome da escola / instituição',placeholder:'Nome completo',tipo:'text'},{label:'CNPJ ou código MEC',placeholder:'Para verificação da instituição',tipo:'text'},{label:'Seu cargo',placeholder:'Ex: Educador especial, coordenador...',tipo:'text'}]}};

function selecionarPerfil(p){_perfilSelecionado=p;document.querySelectorAll('.mo-perfil-btn').forEach(b=>b.classList.remove('sel'));document.getElementById('mpb-'+p).classList.add('sel');const btn=document.getElementById('mo-btn-continuar');btn.style.opacity='1';btn.style.pointerEvents='auto';}
function moAvancarPlanos(){if(!_perfilSelecionado) return;if(_perfilSelecionado==='familia'){fecharMo();ir('tela-planos');mostrarPlanosPorPerfil('familia');return;}const cfg=_credenciais[_perfilSelecionado];document.getElementById('mo-etapa2-titulo').textContent=cfg.titulo;document.getElementById('mo-etapa2-sub').textContent=cfg.sub;document.getElementById('mo-etapa2-campos').innerHTML=cfg.campos.map(c=>`<div class="cg"><label class="cl">${c.label}</label><input type="${c.tipo}" class="ci" placeholder="${c.placeholder}"></div>`).join('');document.getElementById('mo-etapa1').style.display='none';document.getElementById('mo-etapa2').style.display='block';lucide.createIcons();}
function moVoltarEtapa(){document.getElementById('mo-etapa1').style.display='block';document.getElementById('mo-etapa2').style.display='none';}
function moVoltarEtapaDesktop(){document.getElementById('mo-etapa-desktop').style.display='none';document.getElementById('mo-etapa1').style.display='block';}
function entrarAreaProfissional(){fecharMo();ir('tela-planos');mostrarPlanosPorPerfil('profissional');}

/* ── Override ir() ── */
const _irBase=ir;
ir=function(id){
    _irBase(id);
    if(id==='tela-linhatempo') setTimeout(()=>{iniciarLT();lucide.createIcons();},50);
...
function criarTelaRedeLegado() {
    ...
    tela.innerHTML = `
        ...
        <div class="ni" style="flex-shrink:0">
            <div class="nit" data-destino="tela-painel"><i data-lucide="layout-grid"></i><span>Painel</span></div>
            <div class="nit at"><i data-lucide="users"></i><span>Rede</span></div>
            <div class="nit nit-lt" data-destino="tela-linhatempo"><i data-lucide="hourglass"></i><span>Tempo</span></div>
            <div class="nit" data-destino="tela-partilhar"><i data-lucide="share-2"></i><span>Partilhar</span></div>
            <div class="nit" data-destino="tela-ajustes"><i data-lucide="settings"></i><span>Ajustes</span></div>
        </div>`;
    document.querySelector('.app').appendChild(tela);
    lucide.createIcons();
