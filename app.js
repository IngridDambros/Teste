...
let _agDiaSel = new Date().getDate();
let _agCorSel = 'azul';

const AG_MESES = ['JANEIRO','FEVEREIRO','MARÇO','ABRIL','MAIO','JUNHO','JULHO','AGOSTO','SETEMBRO','OUTUBRO','NOVEMBRO','DEZEMBRO'];
const AG_MESES_CURTOS = ['jan','fev','mar','abr','mai','jun','jul','ago','set','out','nov','dez'];

function renderAgenda() {
    const hoje = new Date();
    document.getElementById('ag-mes-nome').textContent = AG_MESES[_agMes] + ' ' + _agAno;

    const primeiroDia = new Date(_agAno, _agMes, 1).getDay();
    const diasNoMes   = new Date(_agAno, _agMes + 1, 0).getDate();
    const diasMesAnt  = new Date(_agAno, _agMes, 0).getDate();
    ...
    for (let i = 0; i < totalCells; i++) {
        ...
        if (i < primeiroDia) {
            dia = diasMesAnt - primeiroDia + i + 1; outro = true;
        } else if (i >= primeiroDia + diasNoMes) {
            dia = i - primeiroDia - diasNoMes + 1; outro = true;
        } else {
            dia = i - primeiroDia + 1;
            if (dia === hoje.getDate() && _agMes === hoje.getMonth() && _agAno === hoje.getFullYear())
                div.classList.add('hoje');
            if (dia === _agDiaSel) div.classList.add('selecionado');

            const key = `${_agAno}-${String(_agMes+1).padStart(2,'0')}-${String(dia).padStart(2,'0')}`;
            if (agenda[key] && agenda[key].length > 0) {
                const dotsDiv = document.createElement('div');
                dotsDiv.className = 'ag-dots';
                [...new Set(agenda[key].map(t => t.cor || 'azul'))].slice(0,3).forEach(c => {
                    const d = document.createElement('span');
                    d.className = `ag-dot ag-dot-${c}`;
        ...
        if (outro) div.classList.add('outro-mes');

        const num = document.createElement('span');
        num.className = 'ag-dia-num';
        num.textContent = dia;
        div.insertBefore(num, div.firstChild);

        if (!outro) div.addEventListener('click', ((d) => () => { _agDiaSel = d; renderAgenda(); })(dia));
...
function agMover(dir) {
    _agMes += dir;
    if (_agMes < 0)  { _agMes = 11; _agAno--; }
    if (_agMes > 11) { _agMes = 0;  _agAno++; }
    _agDiaSel = 1;
    renderAgenda();
}
...
function agRenderLista() {
    const la     = JSON.parse(localStorage.getItem('la') || '{}');
    const agenda = la.agenda || {};
    const key    = `${_agAno}-${String(_agMes+1).padStart(2,'0')}-${String(_agDiaSel).padStart(2,'0')}`;
    const tarefas = (agenda[key] || []).slice().sort((a,b) => a.hora.localeCompare(b.hora));

    const dataEl = document.getElementById('ag-rotina-data');
    if (dataEl) dataEl.textContent = _agDiaSel + ' de ' + AG_MESES_CURTOS[_agMes];

    const lista = document.getElementById('ag-lista');
    if (!lista) return;

    if (tarefas.length === 0) {
        lista.innerHTML = '<div class="ag-vazio"><i data-lucide="calendar-x"></i><p>Nenhuma tarefa para este dia</p></div>';
    } else {
        lista.innerHTML = tarefas.map((t, i) => `
            <div class="ag-tarefa">
                <span class="ag-tarefa-hora">${t.hora}</span>
                <div class="ag-tarefa-dot ag-dot-${t.cor||'azul'}"></div>
                <span class="ag-tarefa-texto">${t.texto}</span>
                <button class="ag-tarefa-del" onclick="agDeletar('${key}',${i})"><i data-lucide="x"></i></button>
            </div>`).join('');
    }
    lucide.createIcons();
}
...
function agSelCor(btn, cor) {
    document.querySelectorAll('.ag-cor').forEach(b => b.classList.remove('sel'));
    btn.classList.add('sel');
    _agCorSel = cor;
}
function agSalvarTarefa() {
    const hora  = document.getElementById('ag-inp-hora').value;
    const texto = document.getElementById('ag-inp-texto').value.trim();
    if (!hora || !texto) { alert('Preencha o horário e a tarefa.'); return; }
    const la = JSON.parse(localStorage.getItem('la') || '{}');
    if (!la.agenda) la.agenda = {};
    const key = `${_agAno}-${String(_agMes+1).padStart(2,'0')}-${String(_agDiaSel).padStart(2,'0')}`;
    if (!la.agenda[key]) la.agenda[key] = [];
    la.agenda[key].push({ hora, texto, cor: _agCorSel });
    localStorage.setItem('la', JSON.stringify(la));
    document.getElementById('ag-inp-hora').value = '';
    document.getElementById('ag-inp-texto').value = '';
    agFecharAdd();
    renderAgenda();
}
...
function abrirNivel(n) {
    ...
    document.getElementById('tela-nivel' + n).style.transform = 'translateY(0)';
    if (n === 1) atuConts();
    lucide.createIcons();
}
