...
const _EXP_DATA = {
    'pp-expand-edu': [
        { label: '🏫 Matriculado', items: [
            { icon: 'school',         nome: 'Escolas e Instituições',    fn: "toggleEduInline('form-escola', this)" },
            { icon: 'users',          nome: 'Mediadores e Professores',  fn: "toggleEduInline('form-mediador', this)" },
            { icon: 'file-text',      nome: 'Plano de Ensino Individual',fn: "toggleEduInline('form-pei', this)" },
            { icon: 'star',           nome: 'Conquistas e Marcos',       fn: "toggleEduInline('form-conquista', this)" },
        ]},
        { label: '⏳ Fora da escola', items: [
            { icon: 'alert-triangle', nome: 'Motivo do Afastamento',     fn: "toggleEduInline('form-barreira', this)" },
            { icon: 'clock',          nome: 'Lacuna Escolar',            fn: "toggleEduInline('form-lacuna', this)" },
            { icon: 'sun',            nome: 'Atividades do Dia a Dia',   fn: "toggleEduInline('form-ativ-diaria', this)" },
            { icon: 'award',          nome: 'Marcos de Autonomia',       fn: "toggleEduInline('form-avd', this)" },
        ]},
        { label: '🏠 Adaptado', items: [
            { icon: 'home',           nome: 'Local / Instituição',       fn: "toggleEduInline('form-escola', this)" },
            { icon: 'users',          nome: 'Educadores e Facilitadores',fn: "toggleEduInline('form-mediador', this)" },
            { icon: 'sun',            nome: 'Plano de Atividades',       fn: "toggleEduInline('form-ativ-diaria', this)" },
            { icon: 'star',           nome: 'Conquistas e Marcos',       fn: "toggleEduInline('form-conquista', this)" },
        ]},
    ],
    'pp-expand-familia': [
...
function expandirCardEdu(el)       { _expandirCard(el, 'pp-expand-edu'); }
...
function mudarEduTab(idx)          { _mudarExpTab('pp-expand-edu', idx); }

const PP_PANEL_DATA = [
    [ // Raiz
        { icon: 'graduation-cap', titulo: 'Desenvolvimento e Aprendizado', desc: 'Escola, habilidades e conquistas acadêmicas',         fn: "abrirCardEdu(this)" },
        { icon: 'heart',          titulo: 'Memórias e Valores',            desc: 'Registros familiares e pilares de vida',               fn: "abrirCardFamilia(this)" },
        { icon: 'stethoscope',    titulo: 'Dossiê de Saúde',               desc: 'Histórico, diagnósticos e acompanhamentos',            fn: "abrirCardSaude(this)" },
        { icon: 'brain',          titulo: 'Evolução Terapêutica',          desc: 'Progressos e registros das terapias',                  fn: "abrirCardTerapia(this)" },
    ],
    [ // Caminho
        { icon: 'home',      titulo: 'Habilidade de Vida Diária', desc: 'Rotinas, cuidados e independência do dia a dia',    fn: "abrirCardVida(this)" },
        { icon: 'users',     titulo: 'Interação Social',          desc: 'Relações, comunicação e vínculos',                   fn: "abrirCardSocial(this)" },
        { icon: 'activity',  titulo: 'Como foi a regulação?',     desc: 'Emoções e estratégias de autorregulação',            fn: "abrirCardRegulacao(this)" },
        { icon: 'lightbulb', titulo: 'Tomou alguma decisão?',     desc: 'Escolhas, preferências e autonomia',                 fn: "abrirCardDecisao(this)" },
    ],
    [ // Topo
        { icon: 'zap',         titulo: 'Independência Funcional',  desc: 'Capacidades práticas e conquistas de autonomia', fn: "abrirCardIndependencia(this)" },
        { icon: 'globe',       titulo: 'Participação Social',      desc: 'Inclusão, comunidade e convívio',                 fn: "abrirCardParticipacao(this)" },
        { icon: 'trending-up', titulo: 'Autonomia Prática',        desc: 'Tarefas, responsabilidades e protagonismo',       fn: "abrirCardAutonomia(this)" },
        { icon: 'star',        titulo: 'Propósito e Protagonismo', desc: 'Metas, sonhos e direção de vida',                 fn: "abrirCardProposito(this)" },
    ],
    [ // Essência
        { icon: 'sun',    titulo: 'Pequenas Alegrias',        desc: 'Momentos, descobertas e bem-estar emocional', fn: "abrirCardAlegria(this)" },
        { icon: 'repeat', titulo: 'Rotina de Cuidado',        desc: 'Hábitos, cuidados e equilíbrio diário',        fn: "abrirCardRotinaCuidado(this)" },
        { icon: 'shield', titulo: 'Protocolo de Apoio',       desc: 'Estratégias e planos de suporte',              fn: "abrirCardProtocolo(this)" },
        { icon: 'users',  titulo: 'Rede de Cuidado e Futuro', desc: 'Família, profissionais e planejamento futuro', fn: "abrirCardRede(this)" },
    ]
];
...
function selecionarNivelTab(idx) {
    ...
    document.querySelectorAll('.pp-nivel-tab').forEach((t, i) =>
        t.classList.toggle('pp-nivel-tab-at', i === idx)
    );
    const stack = document.getElementById('pp-content-cards');
    if (!stack) return;
    const raizIds = ['card-edu', 'card-familia', 'card-saude', 'card-terapia'];
    stack.innerHTML = PP_PANEL_DATA[idx].map((c, i) => `
        <div class="pp-panel-card ${cardCores[i]}"${idx === 0 ? ` id="${raizIds[i]}"` : ''} onclick="${c.fn}">
            <div class="pp-pc-topo">
                <h2 class="pp-pc-titulo">${c.titulo}</h2>
                <i data-lucide="${c.icon}" style="width:20px;height:20px;stroke:rgba(255,255,255,0.65);stroke-width:2;flex-shrink:0"></i>
            </div>
            <p class="pp-pc-desc">${c.desc}</p>
        </div>
    `).join('');
    lucide.createIcons();
}
